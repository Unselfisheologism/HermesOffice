...
import {
  hermesofficeApiKey,
  hermesofficeAuthPath,
  hermesofficeLoginInFlight,
  hermesofficeLogout,
  hermesofficeProxyFallbackPreferred,
  loadGenofficeAuth,
  resetGenofficeAuthCache,
  startGenofficeLogin,
  type GskLoginProgress,
} from '../src/hermesoffice-auth'
...
beforeEach(() => {
  dir = mkdtempSync(join(tmpdir(), 'hermesoffice-auth-'))
  process.env.HERMESOFFICE_AUTH_DIR = dir
...
afterEach(() => {
  ...
  delete process.env.HERMESOFFICE_AUTH_DIR
  delete process.env.GSK_API_KEY
  setGskProxyUrl('')
  resetGenofficeAuthCache()
})
  ...
  const fetchMock = vi.fn(async (input: string | URL, _init?: RequestInit) => {
    ...
    if (url.includes('/api_tokens/create')) {
      return jsonResponse(
        opts.createResponse ?? {
          status: 0,
          data: { key_id: 'kid-1', key_name: 'hermesoffice', token: 'gsk-hermesoffice-key' },
        },
      )
    }
...
describe('startGenofficeLogin', () => {
  it('runs device_code → poll → session → key create and stores the key', async () => {
    ...
    const saved = JSON.parse(readFileSync(hermesofficeAuthPath(), 'utf-8'))
    expect(saved).toEqual({
      api_key: 'gsk-hermesoffice-key',
      key_id: 'kid-1',
    ...
    expect(JSON.parse(String(init.body))).toEqual({ key_name: 'hermesoffice' })
  })

  it('feeds gskApiKey(), losing only to an explicit GSK_API_KEY env override', async () => {
    stubFlow()
    await loginAndCollect()
    expect(gskApiKey()).toBe('gsk-hermesoffice-key')
  ...
  it('retries through the registered proxy channel when the direct fetch cannot connect', async () => {
    ...
    expect(events.at(-1)).toEqual({ phase: 'success' })
    expect(hermesofficeApiKey()).toBe('gsk-hermesoffice-key')
    expect(hermesofficeProxyFallbackPreferred()).toBe(true)
  })
  ...
  it('treats a gateway status (502) as channel failure: fails over without adopting the channel', async () => {
    ...
    expect(hermesofficeProxyFallbackPreferred()).toBe(false)
  })
  ...
  it('re-login revokes the superseded key (best-effort)', async () => {
    ...
    const fetchMock = stubFlow({
      createResponse: { status: 0, data: { key_id: 'kid-2', token: 'gsk-hermesoffice-key-2' } },
    })
    await loginAndCollect()
    expect(loadGenofficeAuth()).toMatchObject({ apiKey: 'gsk-hermesoffice-key-2', keyId: 'kid-2' })
    await vi.waitFor(() => {
  ...
  it('a new login cancels the in-flight one without emitting on it', async () => {
    ...
    expect(hermesofficeLoginInFlight()).toBe(true)
...
describe('hermesofficeLogout', () => {
  it('revokes the key server-side and removes the local file', async () => {
    const fetchMock = stubFlow()
    await loginAndCollect()

    await hermesofficeLogout()
    expect(existsSync(hermesofficeAuthPath())).toBe(false)
    expect(hermesofficeApiKey()).toBe('')
