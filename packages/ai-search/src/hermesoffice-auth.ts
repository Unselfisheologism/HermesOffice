/**
 * HermesOffice's own Genspark identity: device-code login (office_addin_auth,
 * app_type=hermesoffice) minting a gsk API key named "hermesoffice" — the key_name
 * lands in billing as billing_tag, attributing all traffic (incl. gsk CLI
 * subprocesses) to HermesOffice. Stored in ~/.hermesoffice/auth.json, deliberately
...
const APP_TYPE = 'hermesoffice'
const KEY_NAME = 'hermesoffice'
...
async function proxyFallbackFetch(): Promise<typeof fetch | null> {
  ...
  if (process.versions.electron) {
    try {
      ...
      if (session) {
        const ses = session.fromPartition('hermesoffice-login-proxy')
...
/** Override dir via HERMESOFFICE_AUTH_DIR (test isolation). */
export function hermesofficeAuthPath(): string {
  return join(process.env.HERMESOFFICE_AUTH_DIR || join(homedir(), '.hermesoffice'), 'auth.json')
}
...
function readAuthFile(): GenofficeAuth | null {
  try {
    const raw = asRecord(JSON.parse(readFileSync(hermesofficeAuthPath(), 'utf-8')))
...
/** The HermesOffice-named api key; '' when not signed in. Cached (invalidated by login/logout). */
export function hermesofficeApiKey(): string {
  return loadGenofficeAuth()?.apiKey ?? ''
}

function saveAuth(auth: GenofficeAuth): void {
  const path = hermesofficeAuthPath()
...
function clearAuth(): void {
  try {
    if (existsSync(hermesofficeAuthPath())) unlinkSync(hermesofficeAuthPath())
  } catch {
...
/**
 * Starts the HermesOffice device-code login, cancelling a previous in-flight one
...
export function hermesofficeLoginInFlight(): boolean {
...
export function ensureGenofficeLogin(openUrl: (url: string) => void): void {
  if (hermesofficeLoginInFlight()) return
...
/**
 * Signs out of HermesOffice only: best-effort server-side revoke of the
 * hermesoffice key, then local removal. The shared gsk CLI login
 * (~/.genspark-tool-cli) is untouched — terminal gsk and Claw keep working.
 */
export async function hermesofficeLogout(): Promise<void> {
...
export function hermesofficeProxyFallbackPreferred(): boolean {
