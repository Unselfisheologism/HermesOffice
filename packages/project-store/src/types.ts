...
// ────────────────────────────────────────────────────────────
// Trusted agent action audit structures (P0/P2)
// ────────────────────────────────────────────────────────────

export type ProposedChangeApp = 'docs' | 'sheets' | 'slides' | 'pdf'

export type ProposedChangeStatus =
  'draft' | 'proposed' | 'accepted' | 'rejected' | 'applied' | 'reverted'

export type ProposedChangeScopeKind =
  'document' | 'block' | 'range' | 'sheet' | 'slide' | 'page' | 'annotation'

export interface ProposedChangeScope {
  kind: ProposedChangeScopeKind
  /** Stable app-specific identifier, e.g. block id, A1 range, slide id, or page number. */
  ref: string
  /** Optional human-readable label rendered in approval UI and timelines. */
  label?: string
}

export interface ProposedChangeOperation {
  id: string
  type: string
  summary: string
  scope: ProposedChangeScope
  /** Redacted, app-specific operation payload. Engines must validate before applying. */
  payload?: unknown
}

export interface ProposedChangeRisk {
  level: 'low' | 'medium' | 'high'
  message: string
}

export interface ProposedChangeActor {
  id: string
  name?: string
...
export interface ProposedChangeRecord {
  ...
  chatId?: string
  filePath?: string
  app: ProposedChangeApp
  status: ProposedChangeStatus
  title: string
  summary: string
  rationale?: string
  actor: ProposedChangeActor
  sessionId?: string
  createdAt: string
  updatedAt: string
  operations: ProposedChangeOperation[]
  risks?: ProposedChangeRisk[]
  /** Optional compact preview/diff text. Keep full app previews outside project-store. */
  preview?: string
}
...
export interface ProjectDocumentReference {
  ...
  relation: 'derived-from' | 'summarizes' | 'references' | 'exports-to' | 'meeting-minutes-for'
  createdAt: string
  updatedAt: string
  note?: string
}
