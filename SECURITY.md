...
[private vulnerability reporting](https://github.com/genspark-ai/hermesoffice/security/advisories/new)
...
- Every `shell.openExternal` call goes through a single shared gate
  (`@hermesoffice/electron-utils` → `safeExternalUrl`) that parses the URL and
