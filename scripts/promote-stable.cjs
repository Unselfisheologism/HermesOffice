 ...
 * beta-publish time (HermesOffice-mac-arm64-<v>.yml / HermesOffice-win-<v>.yml) is
 ...
 *
 * The marketing download aliases (HermesOffice.dmg / HermesOfficeSetup.exe) also
 ...
 *
 * Requires: az CLI + AZURE_STORAGE_CONNECTION_STRING + HERMESOFFICE_UPDATE_URL
...
const PLATFORMS = [
  {
    flag: '--mac',
    archive: (v) => `HermesOffice-mac-arm64-${v}.yml`,
    feed: 'latest-mac.yml',
    betaFeed: 'beta-mac.yml',
    installer: (v) => `HermesOffice-${v}-arm64.dmg`,
    alias: 'HermesOffice.dmg',
  },
  {
    flag: '--win',
    archive: (v) => `HermesOffice-win-${v}.yml`,
    feed: 'latest.yml',
    betaFeed: 'beta.yml',
    installer: (v) => `HermesOfficeSetup-v${v}.exe`,
    alias: 'HermesOfficeSetup.exe',
  },
]

function channelTarget() {
  const raw = process.env.HERMESOFFICE_UPDATE_URL
  if (!raw) fatal('HERMESOFFICE_UPDATE_URL env not set')
  const base = raw.replace(/\/+$/, '')
  let segments
  try {
    segments = new URL(base).pathname.replace(/^\/+/, '').split('/')
  } catch {
    fatal('HERMESOFFICE_UPDATE_URL is not a valid URL')
  }
  const container = segments[0]
  const prefix = segments.slice(1).join('/')
  if (!container || !prefix) {
    fatal('HERMESOFFICE_UPDATE_URL must look like https://<cdn-host>/<container>/<prefix>')
  }
