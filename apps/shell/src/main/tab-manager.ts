...
export class TabManager {
  private readonly tabs: TabRecord[] = [
    { id: HOME_ID, kind: 'home', view: null, title: 'HermesOffice' },
  ]
  ...
  constructor(
    ...
    // See https://github.com/genspark-ai/hermesoffice/issues/15
  ...
  openDocsTab(openPath?: string, options?: { newBlank?: boolean }): string {
    ...
    this.tabs.push({
      ...
      kind: 'docs',
      view,
      title: openPath ? basename(openPath) : this.untitled('docs', 'HermesOffice Docs'),
      filePath: openPath,
    })
    this.activateTab(id)
    return id
