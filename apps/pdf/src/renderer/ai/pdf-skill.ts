...
- Reading fallback (fork): if the page tools (search_text/read_pages) are NOT available in your session, read the document with the hermesoffice_extract_text tool, passing the Document file path below — never claim you cannot read the document when a file path is available.
  ...
  return {
    ...
    buildContext: () => {
      const parts = [
        ...
        // Fork: caminho absoluto — permite leitura via engine (hermesoffice_extract_text)
